...
- None beyond what InsForge's payments setup already manages. The Stripe test product and price are provisioned as InsForge payments configuration. The concrete catalog IDs should stay in environment/provider configuration rather than public source; Feature 2 reads the Pro monthly price through `STRIPE_PRO_MONTHLY_PRICE_ID`, following the InsForge payments path documented in `context/library-docs.md`.
