...
Server `app/find-jobs/page.tsx` follows the exact `DashboardPage`/`ProfilePage` auth pattern (`createInsforgeServer`, redirect to `/login?error=session` if no session), then renders the shared `Navbar` and the client `FindJobsPage` component. `components/find-jobs/FindJobsPage.tsx` owns one piece of state, `hasSearched`, set by the search form's `onSubmit`; the results card (banner, filter bar, table, pagination) only renders once `hasSearched` is true. Per spec [0005](../docs/specs/0005-find-jobs-page-ui/index.md), everything below the search form is intentionally static: the filter input, both `<select>` dropdowns, and every pagination button render exactly as designed but carry no `onChange`/`onClick` wiring, and the 6 row dataset comes from a fixed `lib/mock-jobs.ts` array, never filtered or sorted. Feature 11 (Filter + Sort + Pagination) is the feature that wires this markup to real behavior against real `jobs` table data; this page's job was the visual build only. The match score color bands here (green ≥90%, blue 80 to 89%, orange below 80%) are specific to this page's fixed mock values and are a different concern from feature 11's own High/Low Match semantics (`>= 70%` / `< 70%`) — don't conflate the two if reusing this bar elsewhere.
