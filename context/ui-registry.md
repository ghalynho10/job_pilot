...
Last updated: 2026-07-31 (feature 16, Recent Activity — Real Data)
...
| Activity dot | `size-2 rounded-full`, tone per entry: `bg-accent`, `bg-info-medium`, `bg-success` (real entries only ever use `info` or `success`; `accent` remains a supported token for a future entry type) |
...
Per [spec 0010](../docs/specs/0010-dashboard-page-ui/index.md), feature 14 replaced the old empty-state placeholder shell (a welcome header, a paragraph, and a `DashboardActions` CTA card, now deleted) with the full mock data dashboard from `context/designs/dashboard.png`: four stat cards, a Recent Activity list, and three Recharts charts (`CompanyResearchActivityChart`, `JobsFoundOverTimeChart`, `MatchScoreDistributionChart`), all sourced from `lib/mock-dashboard.ts`. `recharts` (v3) is this project's first charting dependency; every chart colors itself from the existing `--color-*` CSS variable tokens rather than a separate chart palette. Feature 15 wired the four stat cards to real data per `context/build-plan.md`: `app/dashboard/page.tsx` now also reads the current user's `jobs` rows (`match_score, company_research, found_at`, scoped by `user_id`), and `lib/dashboard-stats.ts`'s pure `computeDashboardStats` derives Total Jobs Found (row count), Avg. Match Rate (mean of non-null `match_score`, `"N/A"` when no job has been scored yet), Companies Researched (count of non-null `company_research`), and Jobs This Week (`found_at` within the last 7 days) from that one query. `mockStats` was removed from `lib/mock-dashboard.ts` as superseded; the mock "vs last week" trend pills had no real data source behind them, so real stat cards use a plain caption instead of a trend pill (the `StatCard` trend branch is now unused but left in place since it's still a shared, generically useful display mode). Feature 16 wired Recent Activity to real data, also per `context/build-plan.md`: `app/dashboard/page.tsx` now also reads the current user's completed `agent_runs` (`id, job_title_searched, jobs_found, completed_at`) and `jobs` rows with a non null `company_research_completed_at` (`id, company, company_research_completed_at`), both scoped by `user_id` and ordered newest first, capped at 10 rows each. `lib/dashboard-activity.ts`'s pure `computeRecentActivity` merges the two sources by timestamp, keeps the newest 8 combined, and formats each into a `DashboardActivityEntry`: an agent run becomes `"Found {jobsFound} jobs for {jobTitleSearched}"` with a `success` (green) dot, a researched job becomes `"Researched {company}"` with an `info` (blue) dot, per the build plan's explicit two color rule (accent is not used by any real entry type). Its own `formatTimeAgo` reproduces the mock's "Just now" / "N mins ago" / "N hours ago" / "Yesterday" / "N days ago" phrasing from a raw timestamp. `mockActivity` was removed from `lib/mock-dashboard.ts` as superseded (the `DashboardActivityEntry` type stays, still used by both the real data path and `RecentActivityCard`). Feature 17 wired the three charts to real data per [spec 0011](../docs/specs/0011-analytics-charts-real-data/index.md), reusing the same `statsJobs` query feature 15 already reads (grown to also select `company_research_completed_at`) rather than querying PostHog back out. New `lib/dashboard-charts.ts`: `computeJobsFoundOverTime` (30 day UTC window, zero filled, short date labels e.g. "Jul 15"), `computeMatchScoreDistribution` (all time, the same 5 bands the mock used, nulls and sub-50 scores excluded), `computeCompanyResearchActivity` (rolling 7 day UTC window, zero filled, weekday abbreviation labels). Each of the three chart components now renders the shared empty state (the `FindJobsPage` `bg-surface-secondary`/`role="status"` pattern) in place of the chart when its data set totals 0, and `JobsFoundOverTimeChart`'s `XAxis` gained `interval={4}` so its 30 short date ticks don't collide (the other two, 5 and 7 ticks, needed no change). `mockJobsFoundOverTime`, `mockMatchScoreDistribution`, and `mockCompanyResearchActivity` were removed from `lib/mock-dashboard.ts` as superseded (the `DashboardDayCount`/`DashboardScoreBand` types stay). Feature 14's live verification of the incomplete-profile banner (real signed in throwaway InsForge account, both profile states, desktop and mobile) still stands. Feature 17's `/check verify` passed live on all 8 acceptance criteria, using one throwaway account seeded with jobs spanning the window boundaries and a second with none, which confirmed both the real chart values and all three empty states with zero console errors. Two display level findings from `/check review` (recorded in `docs/reviews/2026-07-31-dashboard-page.md`) were fixed afterward: the 30 day axis used a fixed `interval={4}`, which thins to indices 0/5/10/15/20/25 and so never labeled today, making the axis read as ending about five days early. It now uses `interval="preserveStartEnd"`, recharts' own auto option for overlapping labels, which spaces ticks by available width and always keeps both ends. Prefer `preserveStartEnd` over a fixed number for any wide axis here: a hardcoded step both drops the final tick and fails to adapt when the card is narrower. Separately, `utcDateKey` called `.toISOString()` on an unvalidated value, so one malformed timestamp would throw `RangeError` and fail the whole dashboard render; row timestamps now go through `rowDateKey`, which drops just that row, matching how `dashboard-stats.ts` and `dashboard-activity.ts` already degrade. Feature 16's `/check verify` also passed live, against a throwaway account seeded with completed `agent_runs` and researched `jobs` deliberately interleaved in time: the rendered list alternated run, research, run, research in exact timestamp order, which proves the two sources are genuinely merged and sorted rather than concatenated. Known, accepted divergence (not a bug, per `docs/reviews/2026-07-31-dashboard-page.md`): the three charts window on UTC calendar days (`[today-(N-1), today]` UTC), while the "Jobs This Week" stat card windows on a rolling 168 hours from `now`. Both describe "recent," both sit on the same screen, and their totals will not always agree — a user off UTC will routinely see the chart's last plotted day differ from their own local "today." Spec 0011 explicitly chose UTC for the charts; this was never reconciled against feature 15's rolling window and is left as-is by choice, not fixed here.
